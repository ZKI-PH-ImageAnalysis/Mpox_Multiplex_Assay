...
@jit(nopython=True)    
def predict_degree_fast(x, ant_matrix, cons_vect, weights, part_matrix):
    """
    ...
    """
    sample_size = x.shape[0]
    degree_values = np.zeros(sample_size)
    # For each sample
    for i in range(sample_size):
        best_match_index = 0
        best_match = 0.0
...
class FuzzyRuleBasedClassifier():
    ...
    def predict_degree(self, x):
        return predict_degree_fast(x, self.ant_matrix, self.cons_vect, self.weights, self.part_matrix)
    ...
    def conf_degree(self, x):
        return self.predict_degree(x)
    ...
    def show_partitions(self, inputs, outputs, f=None):
        if f:
            f.write('PARTITIONS\n')
        else:
            print('PARTITIONS')
        ...
        for i, rule in enumerate(self.rules):
            for key in rule.antecedent:
                ...
                labels = self._get_labels(size)
                if f:
                    f.write(str(i+1) + ':\t' + str(size) + '\n')
                    f.write(str(i+1) + ':\t' + str(labels) + '\n')
                else:
                    print(str(i+1) + ':\t' + str(size))
                    print(str(i+1) + ':\t' + str(labels))
                    print(rule.fuzzyset[key]-1)

    def show_DB(self, inputs, f=None):
        """
        ...
        for k, partition in enumerate(self.partitions):
            ...
            print(k, 'anchors', anchors)
            fuzzyset_size= len(anchors) - 2
            print(k, 'fuzzyset_size', fuzzyset_size)
